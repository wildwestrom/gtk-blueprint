...
/// blueprints in the project and their paths.
///
...
pub fn gen_blp_map(input: TokenStream) -> TokenStream {
    ...
    if errors {
        ...
        format!(
            "pub static {BLP_MAP_NAME}: ::gtk_blueprint::phf::Map<&'static str, &'static str> = ::gtk_blueprint{};\n",
            map.build()
        )
        .parse()
        .unwrap()
...
pub fn get_blp(input: TokenStream) -> TokenStream {
    ...
    if std::path::Path::new(&path).exists() {
        format!("{BLP_MAP_NAME}.get(\"{path}\").expect(\"Blueprint did not compile correctly\")")
            .parse()
            .unwrap()
    } else {
        format!("compile_error!(\"Error getting blueprint \\\"{path}\\\": File Not Found.\")")
            .parse()
            .unwrap()
    }
}
